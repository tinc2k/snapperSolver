...
namespace SnapperSolver.Models
{
    public class Game
    {
        ...
        public Game (Matrix matrix)
        {
            ...
            foreach (var block in Matrix.Blocks)
            {
                Particles.Add(new Particle() { Row = block.Row, Column = block.Column, Direction = ParticleDirection.Down, IsActive = false, StartingPoint = Tuple.Create(block.Row, block.Column) });
                Particles.Add(new Particle() { Row = block.Row, Column = block.Column, Direction = ParticleDirection.Right, IsActive = false, StartingPoint = Tuple.Create(block.Row, block.Column) });
                Particles.Add(new Particle() { Row = block.Row, Column = block.Column, Direction = ParticleDirection.Up, IsActive = false, StartingPoint = Tuple.Create(block.Row, block.Column) });
                Particles.Add(new Particle() { Row = block.Row, Column = block.Column, Direction = ParticleDirection.Left, IsActive = false, StartingPoint = Tuple.Create(block.Row, block.Column) });
            }
        }
        public void Move(int row, int column)
        {
            ...
            HitBlock(row, column);
            while (Particles.Any(p => p.IsActive))
            {
                AdvanceTime();
            }
            //Matrix.Log();
        ...
        private void HitBlock(int row, int column)
        {
            ...
            if (block.Type == BlockType.Null)               /*block died?*/
            {
                ...
                foreach(var p in  Particles.Where(x => x.StartingPoint.Item1 == row && x.StartingPoint.Item2 == column))
                {
                    p.IsActive = true;
                }
            }
        ...
        {
            foreach (var particle in Particles.Where(p => p.IsActive).ToList())
            {
                ...
                if ( particle.Row < 1 || particle.Row > Dimensions.NumberOfRows || particle.Column < 1 || particle.Column > Dimensions.NumberOfRows)
                {
                    ...
                    var par = Particles.First(p => p.StartingPoint == particle.StartingPoint && p.Direction == particle.Direction);
